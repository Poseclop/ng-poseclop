...
type TbackEndCardsDetails = {
  [K in TbackEndCardTypes]: {
    title: string;
    icon?: string;
    paragraphs: string[];
    picture: string;
...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    Object.keys(this.cardsDetails).forEach(key => {
      const img = new Image();
      img.src = `/assets/img/${this.cardsDetails[key as TcardTypes].picture}`;
    });
  }
  ...
  onCardClick(type: TcardTypes): void {
    this.smallScreen$.pipe(
      ...
      if (smallScreen) {
        this.bottomSheet.open<ToolBottomSheetComponent, TcardsDetails, void>(ToolBottomSheetComponent, dialogConfig);
      }
    });
  }
}
